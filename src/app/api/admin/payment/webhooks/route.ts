...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = '/api/admin/payment/webhooks' + (queryString ? `?${queryString}` : '');
    const result = await paymentServiceClient.proxyGET(url, authHeader);
    return NextResponse.json(result, { status: 200 });
  } catch (error: any) {
    console.error('API admin webhooks:', error.message);
    return NextResponse.json(
      { success: false, error: error.message || 'Erreur proxy webhooks', total: 0, events: [] },
      { status: 500 }
    );
  }
}
